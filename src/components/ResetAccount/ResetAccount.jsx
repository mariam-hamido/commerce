...
const {setAccessToken} =useContext(UserContext);
 ...
 async function handleResetAccount(formValues)
  {
    setIsLoading(true)
   try {
...
if (data){
  ...
  const validationSchema= Yup.object().shape({
    ...
    newPassword: Yup.string().matches(/^[A-Z][a-z0-9]{2,8}$/,"password should satart with capital letter and contain char&numbers").required(),
...
const formik = useFormik({
  ...
  return (
    ...
    <h1 className='text-4xl font-bold mb-8'>Reset your account password:</h1>
    </div>
    ...
    <div className="relative z-0 w-full mb-5 group">
       ...
       Enter your newPassword
        ...
        {formik.errors.newPassword&& formik.touched.newPassword &&(<span className='text-red-700'>{formik.errors.newPassword}</span>) }
    </div>
